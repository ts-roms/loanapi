...
export class UserService implements IUserService {
  ...
  async create(
    { username, firstName, lastName, email, mobileNumber, role }: UserCreateDto,
  ...
  async active(_id: string, options?: IDatabaseOptions): Promise<UserEntity> {
    ...
    return this.userRepository.updateOneById<UserActiveDto>(_id, dto, options);
  }

  async inactive(_id: string, options?: IDatabaseOptions): Promise<UserEntity> {
    const dto: UserActiveDto = new UserActiveDto();
    dto.isActive = false;
  ...
  async blocked(_id: string, options?: IDatabaseOptions): Promise<UserEntity> {
    const dto: UserBlockedDto = new UserBlockedDto();
    dto.blocked = true;
    dto.blockedDate = this.helperDateService.create();

    return this.userRepository.updateOneById<UserBlockedDto>(_id, dto, options);
  }
